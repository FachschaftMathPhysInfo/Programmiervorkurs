...
Der Compiler für \Cpp, den wir in diesem Kurs benutzen wollen, heißt \swname{g++}.

Dem Compiler übergeben wir die zu kompilierende Datei als Parameter, sowie den Namen der Ausgabedatei (mit einem \param{-o} davor).

Nachdem \swname{g++} uns also ein Maschinencodefile \file{datei} erzeugt hat, können wir es zur Ausführung bringen. Wir tun das, indem wir auf einer shell \file{./datei} eingeben, also einen Punkt, ein Slash und dann den Dateinamen.
...
\begin{enumerate}
    ...
    \item In diesem Verzeichnis liegt eine Datei \texttt{helloworld.cpp}.
        Benutzt \swname{g++}, um diese zu einer Datei \file{hello} zu
        kompilieren.
    \item Führt die Datei \file{hello} aus.
\end{enumerate}
